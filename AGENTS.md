...
This repository is a Codex plugin marketplace — it contains a single `svg` plugin with three SVG generation skills. There is no build system, test runner, or application to run; the "code" is the skill definition files themselves.
...
`.Codex-plugin/marketplace.json` lists the plugin with its source path:
...
description: >
  ...
  Include trigger phrases here so Codex knows when to invoke this skill.
---
```

The skill name comes from the directory name (e.g. `skills/logo/` → `/svg:logo`). Do not add `name:` to the frontmatter.

The body describes how Codex should execute the skill — canvas specs, step-by-step generation instructions, output naming conventions, etc.
