...
This plugin bundles two skills: `book-to-skill` (the generator) and `second-brain`
(a worked example of what the generator produces — the concept map + chapter
toolkits distilled from Tiago Forte's *Building a Second Brain*).
...
Installing the plugin makes **both** skills available: `book-to-skill` (the
generator) and `second-brain` (the example output skill).
